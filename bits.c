 ...
 */
int tmin(void) {
  return (0x80000000 ^ 0xFFFFFFFF)+1;
}
/* 
...
int divpwr2(int x, int n) {
    int s;
    s = ((((x>>31)&0x1)<<31)>>31);
    return ((s^0xFFFFFFFF) & (x >> n)) | (s & ((((x^0xFFFFFFFF)+1)>>n)^0xFFFFFFFF)+1);
}
/* 
 * negate - return -x 
 *   Example: negate(1) = -1.
...
int isLessOrEqual(int x, int y) {
  	int diff = (y + (x ^ 0xFFFFFFFF) + 1);
	return ((!(x >> 31 ^ y >> 31) & !(diff >> 31 & 0x1)) | ((x >> 31 ^ y >> 31) & !(y >> 31 & 0x1)));
	}
/*
